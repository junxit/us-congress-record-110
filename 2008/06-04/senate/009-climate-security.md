---
granule: CRECB-2008-pt8-Pg11300-9
date: 2008-06-04
edition: CRECB
section: SENATE
page: 11300
...
in the future? The problem is, when we use coal, we have CO<INF>2</INF> 
that is emitted into the air. This CO<INF>2</INF> and other greenhouse 
gases contribute significantly to cause global climate change. So we 
need to find a way to capture that CO<INF>2</INF> and to store or 
sequester CO<INF>2</INF> in geological formations or other means.
  How do we use coal in the future? We use coal in the future by being 
able to capture this emitted CO<INF>2</INF>. So how do we do that? The 
...
CO<INF>2</INF>. It is possible. Of course it is possible. At this point 
...
CO<INF>2</INF> in that plant. We capture the CO<INF>2</INF> and use it 
for enhanced oil recovery. It is one of the world's largest examples of 
CO<INF>2</INF> capture at an industrial facility. Half of the 
CO<INF>2</INF> produced at this facility is now captured. This 
CO<INF>2</INF> is put in a pipeline under pressure and sent to 
Saskatchewan, Canada. Oil industry interests there pump it underground 
to enhance oil recovery. We are successfully using CO<INF>2</INF> by 
...
CO<INF>2</INF>? If you are going to unlock the mystery of how you 
...
organization in Arizona is producing CO<INF>2</INF> at a coal-fired 
electric generating plant. What they are doing with it is very 
interesting. They are taking a stream of CO<INF>2</INF> off their stack 
...
grow? It needs two things--sunlight and CO<INF>2</INF>.
...
to a coal-fired electric generating plant. They take the CO<INF>2</INF> 
...
CO<INF>2</INF>, they harvest the algae and produce diesel fuel. So what 
...
electricity and that increased manmade CO<INF>2</INF> is destructive to 
the atmosphere. Yet capturing the CO<INF>2</INF> and producing fuel is 
very beneficial.
...
the CO<INF>2</INF>. Instead of emitting it into the atmosphere, it is 
embedded in the CO<INF>2</INF>. It can be put in a landfill, but you 
...
a very viable way to capture and store the CO<INF>2</INF> underground. 
...
I said previously, of CO<INF>2</INF> being used in marginal oil wells.
...
pump CO<INF>2</INF> down into that ground under pressure, you enhance 
oil recovery. You have a way to get rid of the CO<INF>2</INF> by 
putting pressure on the oil to bring it up. You have gotten rid of the 
CO<INF>2</INF>, protected the environment, are still able to use coal 
...
should be working on ways to take CO<INF>2</INF>, produce baking soda, 
...
Washington into that economy of unprecedented proportions.
  The goal of this legislation is to reduce CO<INF>2</INF> emissions in 
...
our emissions of CO<INF>2</INF>. In short, we have a responsibility to 
lead.
...
warming, from CO<INF>2</INF> emissions in this country in this year. I 
...
on an ``input'' basis. This means allowances to emit CO<INF>2</INF> in 
...
apportion those allowances to emit CO<INF>2</INF>. Why not provide more 
...
in this bill is we provide more allocation to emit CO<INF>2</INF> to 
...
what CO<INF>2</INF> emissions are from our car, truck, and van fleet 
starting in 2005 by incorporating the new CAFE standards for 35 miles 
per gallon by 2020. Here is where we end up in CO<INF>2</INF> emissions 
for cars, trucks, and vans. Great progress. Unfortunately, if we keep 
driving more and more every year, the great reductions in 
CO<INF>2</INF> which could be recognized here are going to end up with 
